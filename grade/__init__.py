...
def output_invalid_for_input_greater_than_100():
  check50.run("python3 main.py").stdin(random.randint(101, 500)).stdout("Invalid")

@check50.check()
def output_invalid_for_input_greater_than_100():
  check50.run("python3 main.py").stdin(random.randint(-100, 1)).stdout("Invalid")

@check50.check()
def random_check_1():
  num = random.randint(1, 101)
  check50.run("python3 main.py").stdin(num).stdout(answer(num))
